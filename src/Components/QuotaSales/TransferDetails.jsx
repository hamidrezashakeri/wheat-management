...
import { getAllTransferQoutaSale, searchTransferQoutaSale } from '../redux/actions/transferQoutaSale';
...
const TransferDetails = () => {
    ...
    const { transfers, numbers } = useSelector(state => state.transferQoutaSale);
    ...
    const handleUpdate = (id)=>{
        ...
        navigate(`/qouta-sales/update-transfer/${id}`, {state: {transfer, type: state.type}});
    }
    ...
    useEffect(() => {
        if(Object.keys(search).length >0){
            dispatch(searchTransferQoutaSale(search, state._id, page, perPage));
        }else{
            dispatch(getAllTransferQoutaSale(state._id, page, perPage));
        }
    ...
    return (
        <React.Fragment>
                                    ...
                                    <tbody>
                                        {transfers.map((t, index) => (
                                            <tr key={t._id}>
                                                ...
                                                <td>{t.driver}</td>
                                                <td>{t.numberTruck}</td>
                                                <td>{t.qoutaSale.rentPrice === '' ? 'پرداخت نشود' : numberWithCommas((Number(t.sourceWeight) * Number(t.qoutaSale.rentPrice)) / 1000)}</td>
                                                {t.qoutaSale.rentPrice === '' ? <td>بدون پرداخت</td> : <td>{t.rentState ? 'پرداخت شده' : (<button className='rent' onClick={() => handleRent(t._id)}>پرداخت</button>)}</td>}
                                            </tr>
            ...
            {openDelete ? <Modal flag={openDelete} setFlag={setOpenDelete} id={id} action="delete" secondId={state._id} />: null}
        </React.Fragment>
    )
}
