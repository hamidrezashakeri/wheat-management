...
import { addTransferQoutaSale } from '../redux/actions/transferQoutaSale';
import CustomDatePicker from '../shared/DatePicker';

const TransferQoutaSales = () => {
    ...
    const handleSubmit = () => {
        ...
        if (validator.current.allValid()) {
            dispatch(addTransferQoutaSale(info, state._id));
            reset();
        } else {
        ...
        <div className='transfer'>
            {state && (
                <div className="transfer-content">
                    <h3 className='transfer-header'>محموله جدید</h3>
                    <div className="transfer-body">
                        ...
                        <div className="input-group">
                            <p>شماره مجوز: <span>{state.number}</span></p>
                        </div>
                        <div className="input-group">
                            <p>انبار مبدا: <span>{state.store.name}</span></p>
                        </div>
                        <div className="input-group">
                            <p>نوع گندم: <span>{state.type.nameProduct}</span></p>
                        </div>
                        <div className="input-group">
                            <label htmlFor="source-weight">وزن مبدا:</label>
                            <input type="text" name='src' value={sourceWeight} onChange={event => setSourceWeight(event.target.value)} />
                            {validator.current.message("src", sourceWeight, "required|numeric:min:0,num")}
                        </div>
                        <div className="input-group">
                            <label htmlFor="destination-weight">وزن مقصد:</label>
                            <input type="text" name='dest' value={destinationWeight} onChange={event => setDestinationWeight(event.target.value)} />
                            {validator.current.message("dest", destinationWeight, "required|numeric:min:0,num")}
                        </div>
                        <div className='input-group'>
                            <label htmlFor="weight-number">شماره توزین:</label>
                            <input type="text" name='wieghtNumber' value={weightNumber} onChange={event => setWeightNumber(event.target.value)} />
                            {validator.current.message("weightNumber", weightNumber, "required|numeric:min:0,num")}
                        </div>
                        <div className="input-group">
                            <label htmlFor="driver">نام راننده:</label>
                            <input type="text" value={driver} onChange={event => setDriver(event.target.value)} />
                        </div>
                        <div className="input-group">
                    ...
                    <button className='submit' onClick={handleSubmit}>ثبت</button>
                </div>
            )}
        </div>
    )
...
export default TransferQoutaSales;