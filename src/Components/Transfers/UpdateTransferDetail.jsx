...
import { updateSingleTransferDetail } from '../redux/actions/transfersDetail';
...
const UpdateTransferDetail = () => {
    ...
    const handleSubmit = () => {
        ...
        if(validator.current.allValid()){
            dispatch(updateSingleTransferDetail(info, state.transferId._id));
            navigate("/transfers");
        }else{
            forceUpdate(1);
    ...
    useEffect(() => {
        setDate(state.date);
        setSourceWeight(state.sourceWeight);
        setDestinationWeight(state.destinationWeight);
        setWeightNumber(state.weightNumber);
        setDriver(state.driver);
        setNumber1(state.numberTruck.substr(0, 2));
        setAlphabet(state.numberTruck.substr(3, 1));
        setNumber2(state.numberTruck.substr(5, 3));
        setNumber3(state.numberTruck.substr(9, 2));
    }, [state])
            ...
            {state && (
                <div className="transfer-content">
                    ...
                    <div className="transfer-body">
                        ...
                        <div className="input-group">
                            <p>شماره مجوز: <span>{state.transferId.number}</span></p>
                        </div>
                        <div className="input-group">
                            <label htmlFor="source-weight">وزن مبدا:</label>
                            <input type="text" value={sourceWeight} onChange={event => setSourceWeight(event.target.value)} />
                        </div>
                        <div className="input-group">
                            <label htmlFor="destination-weight">وزن مقصد:</label>
                            <input type="text" value={destinationWeight} onChange={event => setDestinationWeight(event.target.value)} />
                        </div>
                        <div className="input-group">
                        ...
                        <div className='input-group'>
                            ...
                            <input type="text" value={weightNumber} onChange={event => setWeightNumber(event.target.value)} />
                        </div>
                        <div className="input-group">
...
export default UpdateTransferDetail;